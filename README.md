...
### 5. 11번가 가격 셀렉터 태그 불일치

**문제**  
11번가 크롤러가 111개 DOM 요소를 찾았음에도 결과가 0개. 에러 로그 없음.

**원인 분석**  
`PRICE_SEL = "div.c-card-item__price .value"`로 작성했으나, 실제 DOM에서 해당 요소는 `div`가 아닌 `dd` 태그. CSS 셀렉터가 전혀 매칭되지 않아 가격이 모두 `0`으로 파싱 → `price > 0` 조건에서 전부 필터링.

**해결**  
태그명 제거, 클래스명만 사용하도록 수정.

```python
# Before: 태그 불일치로 매칭 실패
"div.c-card-item__price .value"

# After: 태그 무관하게 클래스로만 선택
...
```json
{
  ...
  "results": [
    ...
    },
    {
      ...
      "price": 335790,
      "reviewCount": 746,
      "link": "https://www.11st.co.kr/products/8653407353",
      "mall": "11번가"
    }
  ]
}
...
서버 실행 후 브라우저에서 검색 UI 바로 사용:

```
http://localhost:8000
```

- 검색어 입력 → Enter 또는 버튼 클릭
- 결과 테이블에서 상품명 클릭 → 해당 쇼핑몰 상품 페이지로 이동
- 쇼핑몰별 색상 구분, 최저가 뱃지 표시
...
```
priceComparison/
├── main.py                  # FastAPI 앱, GET / (웹 UI), GET /search (API)
├── crawlers/
